...
def convert_data(data: List["Station"]) -> None:
    ...
    for n in range(len(data)):
        for m in range(len(data[n])):
            if is_number(data[n][m]):
                if  int(float(data[n][m]))==float(data[n][m]):
                    data[n][m]=int(float(data[n][m]))
                else:
                    data[n][m]=float(data[n][m])
# Data Query Functions


def is_app_only(station: List["Station"]) -> bool:
...
def get_station_info(station_id: int, stations: List["Station"]) -> list:
    ...
    for n in stations:
        if station_id in n:
            lst = [n[NAME],n[BIKES_AVAILABLE],n[DOCKS_AVAILABLE],not (is_app_only(n))]
            return lst
    return lst
...
def get_nearest_station(my_latitude: float, my_longitude: float,
    ...
    7486
    """
    d1 = 3.14*EARTH_RADIUS
    id = -1
    for n in stations:
        d2=get_lat_lon_distance(my_latitude,my_longitude,n[LATITUDE],n[LONGITUDE])
        if d2<d1:
            d1 = d2
            id = int(n[ID])
    return id
...
def upgrade_stations(threshold: int, num_bikes: int,
    ...
    bikes_added=0
    for n in stations:
        if n[CAPACITY]<threshold:
            bikes_added += num_bikes
            n[CAPACITY] += num_bikes
            n[BIKES_AVAILABLE] += num_bikes
    return bikes_added
