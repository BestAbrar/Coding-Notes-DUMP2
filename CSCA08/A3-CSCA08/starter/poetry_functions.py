...
def get_syllable_count(poem_line: POEM_LINE,
    ...
    """
    syllables = 0
    poem_line_words = poem_line.split()
    for n in range(len(poem_line_words)):
        poem_line_words[n]=transform_string(poem_line_words[n])
    sylables_in_word = {}
    for n in words_to_phonemes:
        sylables_in_word[n]=0
        for m in words_to_phonemes[n]:
            if is_vowel_phoneme(m):
                sylables_in_word[n] +=1
    for n in poem_line_words:
        syllables += sylables_in_word[n]
    return syllables
...
def check_syllable_counts(poem_lines: POEM,
    ...
    """
    lst=[]
    for n in range(len(poem_lines)):
        if get_syllable_count(poem_lines[n],word_to_phonemes) != \
           description[0][n] and description[0][n] != 0:
            lst.append(poem_lines[n])
    return lst
...
def get_last_syllable(word_phonemes: PHONEMES) -> PHONEMES:
    ...
    """
    tmp=-1
    for n in word_phonemes:
        if is_vowel_phoneme(n):
            tmp = word_phonemes.index(n)
    if tmp==-1:
        return ()
    return word_phonemes[tmp:]
...
def words_rhyme(word1: str, word2: str, word_to_phonemes: PRONUNCIATION_DICT) \
    ...
    False
    """
    return get_last_syllable(word_to_phonemes[transform_string(word1)]) == \
           get_last_syllable(word_to_phonemes[transform_string(word2)])
...
def all_lines_rhyme(poem_lines: POEM, lines_to_check: List[int],
    ...
    if len(lines_to_check)==1:
        return True
    for n in range(len(lines_to_check)-1):
        if not words_rhyme(poem_lines[lines_to_check[n]].split()[-1]    \
                       ,poem_lines[lines_to_check[n+1]].split()[-1],\
                       word_to_phonemes):
            return False
    return True
def get_symbol_to_lines(rhyme_scheme: Tuple[str]) -> Dict[str, List[int]]:
    ...
    """
    lst = {}
    for n in range(len(rhyme_scheme)):
        if rhyme_scheme[n] not in lst:
            lst[rhyme_scheme[n]] = [n]
        else:
            lst[rhyme_scheme[n]].append(n)
    return lst
...
def check_rhyme_scheme(poem_lines: POEM,
    ...
    """
    lst=[]
    lines_to_check = get_symbol_to_lines(description[1])
    if '*' in lines_to_check:
        lines_to_check.pop('*')
    for n in lines_to_check:
        temp_lst=[]
        if not all_lines_rhyme(poem_lines,lines_to_check[n],word_to_phonemes):
            for m in lines_to_check[n]:
                temp_lst.append(poem_lines[m])
        if len(temp_lst)!=0:
            lst.append(temp_lst)
    return lst
