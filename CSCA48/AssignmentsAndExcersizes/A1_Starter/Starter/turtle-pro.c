...
/*---------------------------------------------------------------------------*/
int isValid(char cmd[], int val){
  if (!strcmp(cmd,"penup")){
    return 1;
  }
  if (!strcmp(cmd,"pendown")){
    return 1;
  }
  if (!strcmp(cmd,"colour")){
    return 2;
  }
...
CmdNode *newCmdNode(char cmd[10], int val) {
   ...
   */
  CmdNode *NewCmdNode =(CmdNode *)calloc(1, sizeof(CmdNode));

  if (isValid(cmd,val)){
    NewCmdNode->val=val;
    strcpy(NewCmdNode->cmd,cmd);
    NewCmdNode->next=NULL;
    NewCmdNode->loop_head=NULL;
    return NewCmdNode;
  }
   ...
   */
   ...
   while(p!=NULL){
     if (p->loop_head!=NULL){
       ...
       p=p->loop_head;
       while(p!=NULL){
       if (isValid(p->cmd,p->val)==2){
         printf("%3d:   %s %d\n", counter,p->cmd,p->val);
       }
       else if(isValid(p->cmd,p->val)==1){
         printf("%3d:   %s\n", counter,p->cmd);
       }
       p=p->next;
       counter++;
   ...
   */
   ...
   while(p!=NULL){
     if(p->loop_head!=NULL){
       ...
       p=p->loop_head;
       while(p!=NULL){
         if (!strcmp(p->cmd,cmd) && isValid(p->cmd,p->val)==2){
           printf("%3d: %s %d\n", counter,p->cmd,p->val);
         }
         else if (p->cmd==cmd && isValid(p->cmd,p->val)==1){
           printf("%3d: %s\n", counter,p->cmd);
         }
         p=p->next;
         counter++;
...
/*---------------------------------------------------------------------------*/
CmdNode *insertCommandLoop(CmdNode *head, CmdNode *new_CmdNode) {
  CmdNode *p = head;
  if (p->loop_head==NULL){
    p->loop_head=new_CmdNode;
    return p;
  }
  p = p->loop_head;
  while (p->next!=NULL){
...
CmdNode *insertCommandBefore(CmdNode *head, CmdNode *new_CmdNode, int cmdNum) {
   ...
   * TODO: Implement this function
   */
   CmdNode *pre = NULL, *tr = NULL;
   int counter=1;

   pre = head;
   tr = pre->next;
   if (cmdNum==0){
     new_CmdNode->next = pre;
     return new_CmdNode;
   }
   while (tr!=NULL && cmdNum != counter){
     pre=tr;
     tr = tr->next;
     counter++;
   }
...
void updateCommand(CmdNode *head, int cmdNum, char cmd[10], int val) {
   ...
   */
   ...
   p=head;
   int counter=0;
   while (p!=NULL && cmdNum != counter){
     p = p->next;
     counter++;
   }
   if (p!=NULL){
     if (isValid(cmd,val)==2){
       strcpy(p->cmd,cmd);
       p->val = val;
     }
     else if (isValid(cmd,val)==1){
       strcpy(p->cmd,cmd);
     }
     printf("Invalid command.\n");
     return;
...
void run(Image *im, CmdNode *head, int *endX, int *endY) {
   ...
   */
   char cmd[10];
   int val = 0, draw=1,direction=0,dirX=1,dirY=0,colour=0,x1=0,y1=0,x2=0,y2=0;
   int loop=0,counter=0;
   CmdNode *p = head;
   CmdNode *temp = NULL, *temp1 = NULL;
   while (p!=NULL){
     strcpy(cmd,p->cmd);
     val = p->val;
     //printf("%s %d\n",cmd,val);
     ...
     counter=0;
     while(counter<loop||counter==0){
     if(temp==NULL){
       counter++;
     }
     if (p==NULL&&loop!=counter){
       p=temp1;
       counter++;
     }
     if (p==temp1&&loop==counter){
       break;
     }
     strcpy(cmd,p->cmd);
     val = p->val;
     ...
     if (isValid(cmd,val)){
       ...
       if (!strcmp(cmd,"forward")){
         if(draw){
          if(x1+(val*dirX)>=SIZEX){
            x2=SIZEX-1;
          }
          else if(x1+(val*dirX)<0){
            x2=0;
          }
          else if(y1+(val*dirY)>=SIZEY){
            y2=SIZEY-1;
          }
          else if(y1+(val*dirY)<0){
            y2=0;
          }else {
            x2=x1+(val*dirX);
            y2=y1+(val*dirY);
