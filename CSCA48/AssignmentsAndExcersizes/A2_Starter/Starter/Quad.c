...
Quad *new_Quad(int tx, int ty, int w, int h, int wsplit, int sx) {
   ...
   */
   Quad *NewQuad = (Quad *)calloc(1, sizeof(Quad));
   NewQuad->tx=tx;
   NewQuad->ty=ty;
   NewQuad->w=w;
   NewQuad->h=h;
   ...
   if (root == NULL)
     return NULL;
      ...
      if(root->right!=NULL && root->left != NULL){
        ...
        root->h=tmp->h;
        root->sx=tmp->sx;
        root->key=tmp->tx + (tmp->ty * tmp->sx);
        root->wsplit=tmp->wsplit;
        BST_delete(root->right,tmp->tx,tmp->ty);
      }
    }
  return root;
...
Quad *delete_BST(Quad *root) {
   ...
   * (consult the Unit 4 notes as needed) (post-order)
   *
...
int get_colour(Image *im, Quad *q) {
   ...
   */
  int avg=0;
  for(int x =q->tx;x<q->w+q->tx;x++){
    for(int y =q->ty;y<q->h+q->ty;y++){
      avg+=im->data[x + (y * q->sx)];
    }
  }
  return avg/(q->w*q->h);
}
...
Quad *split_tree(Image *im, Quad *root, int threshold) {
   ...
   */
    if (root==NULL){
      /*
      Quad *new_quad = new_Quad(0,0,im->sx,im->sx,1,im->sx);
      root=BST_insert(root,new_quad);
      return root;
      */
      return NULL;
    }
    ...
    if(!similar(im,root,threshold)){
      if(root->wsplit==1){
        Quad *new_quad = new_Quad(root->tx+root->w/2,root->ty,root->w/2,root->h,0,im->sx);
        BST_insert(root,new_quad);
        root->w=root->w/2;
        root->wsplit=0;
      }
      else{
        Quad *new_quad = new_Quad(root->tx,root->ty+root->h/2,root->w,root->h/2,1,im->sx);
        BST_insert(root,new_quad);
        root->h=root->h/2;
        root->wsplit=1;
      }
...
void drawOutline(Image *im, Quad *root, unsigned char col) {
   ...
   */
   ...
   for(int x=root->tx;x<root->tx+root->w;x++){
     if (x==root->tx||x==root->tx+root->w-1){
       for(int y =root->ty;y<root->ty+root->h;y++){
         im->data[x + (y * root->sx)]=col;
       }
    }
    else{
      im->data[x+root->sx]=col;
      im->data[x + ((root->h+root->ty-1) * root->sx)]=col;
    }
   }
  return;
}
