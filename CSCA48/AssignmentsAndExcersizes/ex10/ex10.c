...
void find_connected_region_recursion(unsigned char input[SIZEY][SIZEX], int px, int py,
                           unsigned char output[SIZEY][SIZEX], int elevation){

  if(input[py][px]!=elevation||(px<0 || px>=SIZEX || py<0 || py>=SIZEY))
    return;
  output[py][px]=255;
  printf("%d,%d\n",px,py);
  if(!output[py][px-1])
    find_connected_region_recursion(input,px-1,py,output,elevation);
  if(!output[py-1][px])
    find_connected_region_recursion(input,px,py-1,output,elevation);
  if(!output[py][px+1])
    find_connected_region_recursion(input,px+1,py,output,elevation);
  if(!output[py+1][px])
    find_connected_region_recursion(input,px,py+1,output,elevation);
                           ...
                           unsigned char output[SIZEY][SIZEX])
{
   ...
   */
  find_connected_region_recursion(input,px,py,output,input[py][px]);
...
int main()
{
  ...
  find_connected_region(input, 34, 36, output); // Staring position is (24, 16)
